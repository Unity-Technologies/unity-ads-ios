...
final class SDKMetricsValidator {
    ...
    private func extractMetricsWithSorting(from containers: [UnityAdsDiagnosticMetricContainer]) throws -> [UnityAdsDiagnosticMetric] {
        sortedMetrics(containers.flatMap({ $0.metrics }))
    }
    private func receivedMetricContainer() throws -> [UnityAdsDiagnosticMetricContainer] {
        try self.stub.requests.compactMap({ try $0.decodableObject() })
    }

    private func sortedMetrics(_ metrics: [UnityAdsDiagnosticMetric]) -> [UnityAdsDiagnosticMetric] {
