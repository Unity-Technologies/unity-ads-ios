...
- (void)testAddBannerView {
    ...
    UADSBannerView *firstView = [[UADSBannerView alloc] initWithPlacementId: @"firstView"
                                                                       size: CGSizeMake(320, 50)];

    [manager addBannerView: firstView
                bannerAdId: @"firstView"];
...
- (void)testMultipleAddBannerView {
    ...
    UADSBannerView *firstView = [[UADSBannerView alloc] initWithPlacementId: @"firstView"
                                                                       size: CGSizeMake(320, 50)];
    UADSBannerView *secondView = [[UADSBannerView alloc] initWithPlacementId: @"secondView"
                                                                        size: CGSizeMake(320, 50)];

    [manager addBannerView: firstView
                bannerAdId: @"firstView"];
    [manager addBannerView: secondView
                bannerAdId: @"secondView"];
...
- (void)testTriggerBannerDidError {
    ...
    UADSBannerError *unknownError = [[UADSBannerError alloc] initWithCode: UADSBannerErrorCodeUnknown
                                                                 userInfo: @{
    ...
    [manager addBannerView: firstView
                bannerAdId: firstView.viewId];
    [manager triggerBannerDidError: firstView.viewId
                             error: unknownError];
    [self waitForExpectationsWithTimeout: 2
                                 handler: nil];
} /* testTriggerBannerDidError */
...
- (void)testTriggerBannerDidErrorWithNullBanner {
    ...
    [manager triggerBannerDidError: @"test"
                             error: unknownError];
    // test that no crash happens
}
