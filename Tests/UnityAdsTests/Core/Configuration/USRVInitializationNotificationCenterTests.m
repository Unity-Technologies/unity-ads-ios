...
- (void)testCleanupOfWeakDelegateForFailedInit {
    ...
    [self.initializationNotificationCenter triggerSdkInitializeDidFail: @"test"
                                                                  code: 0];

    [self waitForExpectationsWithTimeout: 1
                                 handler: nil];
} /* testCleanupOfWeakDelegateForFailedInit */
...
} /* testTriggerSdkDidInitializeMultipleDelegates */
...
- (void)testTriggerSdkInitializeDidFail {
    ...
    [self.initializationNotificationCenter addDelegate: mock];
    [self.initializationNotificationCenter triggerSdkInitializeDidFail: @"test fail"
                                                                  code: 0];
    [self waitForExpectationsWithTimeout: 1
                                 handler: ^(NSError *error) {
                                 }];
