...
- (void)setUp {
    ...
    [self.privacyMock saveResponse:[UADSInitializationResponse newFromDictionary:@{}]];
...
- (void)test_if_state_is_failed_should_return_null_token_even_if_first_init_token_is_not_nil {
    ...
    [self runTestWithExpectedTokens: @[UADSHeaderBiddingToken.newInvalidToken]
...
- (void)test_adds_prefix_to_native_generated_token {
    ...
    token.uuidString = idGeneratorMock.expectedValue;
    
    [self runTestWithExpectedTokens: @[token]
             expectedGenerationCalled: 0
                            hbTimeout: 1
...
- (void)test_calls_timeout_completion_if_native_generation_is_too_long {
    ...
    [self runTestWithExpectedTokens: (NSArray *)array
             expectedGenerationCalled: 1
                            hbTimeout: 1
                            initState: INITIALIZED_SUCCESSFULLY
                      additionalBlock: nil
...
- (void)test_native_token_generation_timeout_when_privacy_is_not_resolved {
    id sut = [self sutWithPrivacyWait:true timeout: 4];
...
- (void)test_native_token_generation_returns_token_when_privacy_is_resolved_after_token_requested {
    ...
    id sut = [self sutWithPrivacyWait:true timeout: waitForPrivacy];
...
- (id<UADSHeaderBiddingAsyncTokenReader, UADSHeaderBiddingTokenCRUD>)sutWithPrivacyWait: (BOOL)tsi_prw
                                                                                timeout: (NSInteger)privacyTimeout {
    USRVConfiguration *config =  [USRVConfiguration new];
    config.experiments = [UADSConfigurationExperiments newWithJSON: @{ @"tsi_prw": @(tsi_prw).stringValue }];
...
- (void)runTestWithExpectedTokens: (NSArray<UADSHeaderBiddingToken *> *)expectedTokens
    ...
    [self runTestWithExpectedTokens: expectedTokens
...
//    [[UADSServiceProviderContainer.sharedInstance.serviceProvider configurationSaver] saveConfiguration: config];
}
