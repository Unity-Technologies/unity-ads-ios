...
- (BOOL)waitForViewControllerStart {
    ...
    [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController: self.viewController
                                                                                 animated: true
                                                                               completion: nil];
...
- (void)testPrepare {
    ...
    dispatch_async(queue, ^{
        [self.videoPlayer prepare: [TestUtilities getTestVideoUrl]
                    initialVolume: 1.0f
                          timeout: 10000];
    });
...
- (void)testPrepareAndPlayNonExistingUrl {
    ...
    dispatch_async(queue, ^{
        [self.videoPlayer prepare: invalidVideoUrl
                    initialVolume: 1.0f
                          timeout: 10000];
    });
    ...
    [self.videoView removeFromSuperview];
    XCTAssertTrue([self waitForViewControllerExit], @"Couldn't exit viewController properly");
} /* testPrepareAndPlayNonExistingUrl */
...
- (void)testPreparePlaySeekToPause {
    ...
    [self performSelector: @selector(seekVideoPlayerTo4080)
               withObject: self
               afterDelay: 1];
...
- (void)testPreparePlayStop {
    ...
    [self performSelector: @selector(stopVideoPlayer)
               withObject: self
               afterDelay: 2];
...
- (void)testPreparePlaySetVolumePause {
    ...
    [self performSelector: @selector(setVideoPlayerVolumeTo0666)
               withObject: self
               afterDelay: 2];
...
- (void)testPreparePlayPause {
    ...
    [self performSelector: @selector(pauseVideoPlayer)
               withObject: self
               afterDelay: 2];
