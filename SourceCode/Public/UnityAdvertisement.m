...
+ (void)initialize: (NSString *)gameId testMode: (BOOL)testMode enablePerPlacementLoad: (BOOL)enablePerPlacementLoad {
    [self initialize: gameId
                      delegate: nil
                      testMode: testMode
        enablePerPlacementLoad: enablePerPlacementLoad];
}

+ (void)initialize: (NSString *)gameId delegate: (nullable id<UnityAdsDelegate>)delegate testMode: (BOOL)testMode enablePerPlacementLoad: (BOOL)enablePerPlacementLoad {
...
+ (void)initialize: (NSString *)gameId testMode: (BOOL)testMode enablePerPlacementLoad: (BOOL)enablePerPlacementLoad initializationDelegate: (id<UnityAdsInitializationDelegate>)initializationDelegate {
    [UnityServices  initialize: gameId
                      delegate: [[UnityServicesListener alloc] init]
                      testMode: testMode
           usePerPlacementLoad: enablePerPlacementLoad
        initializationDelegate: initializationDelegate];
}
...
+ (void)show: (UIViewController *)viewController {
    if ([UADSPlacement getDefaultPlacement]) {
        [UnityAds  show: viewController
            placementId: [UADSPlacement getDefaultPlacement]];
    } else {
        [self handleShowError: @""
                unityAdsError: kUnityAdsErrorNotInitialized
                      message: @"Unity Ads default placement is not initialized"];
    }
}
...
+ (void)handleShowError: (NSString *)placementId unityAdsError: (UnityAdsError)unityAdsError message: (NSString *)message {
    ...
    [UnityAdsDelegateUtil unityAdsDidError: unityAdsError
    ...
    if (placementId) {
        ...
        [UnityAdsDelegateUtil unityAdsDidFinish: @""
                                withFinishState: kUnityAdsFinishStateError];
    }
}
