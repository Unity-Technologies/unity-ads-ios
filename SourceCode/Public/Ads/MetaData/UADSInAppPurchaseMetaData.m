...
- (void)setReceiptPurchaseData: (NSString *)receiptPurchaseData {
    [self set: @"receiptPurchaseData"
        value : receiptPurchaseData];
}
...
- (void)commit {
    if ([USRVStorageManager init]) {
        ...
        if (self.storageContents && storage) {
            ...
            [storage set: @"iap.purchases"
                   value : purchases];
            [storage writeStorage];
            [storage sendEvent: @"SET"
                        values: [storage getValueForKey: @"iap.purchases"]];
        } else {
