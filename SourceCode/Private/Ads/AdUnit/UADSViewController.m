...
- (instancetype)initWithViews: (NSArray *)views supportedOrientations: (NSNumber *)supportedOrientations statusBarHidden: (BOOL)statusBarHidden shouldAutorotate: (BOOL)shouldAutorotate isTransparent: (BOOL)isTransparent homeIndicatorAutoHidden: (BOOL)homeIndicatorAutoHidden {
    ...
    [[USRVWebViewApp getCurrentApp] sendEvent: UADSNSStringFromAdUnitEvent(kUnityAdsViewControllerInit)
...
- (void)viewDidLoad {
    ...
    [[USRVWebViewApp getCurrentApp] sendEvent: UADSNSStringFromAdUnitEvent(kUnityAdsViewControllerDidLoad)
                                     category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryAdunit)
                                       param1: nil];
}
...
- (void)viewDidAppear: (BOOL)animated {
    ...
    if (_viewHandlers) {
        for (NSString *key in _viewHandlers) {
            ...
            if (viewHandler) {
                [viewHandler viewDidAppear: self
                                  animated: animated];
            }
        }
    }

    [[USRVWebViewApp getCurrentApp] sendEvent: UADSNSStringFromAdUnitEvent(kUnityAdsViewControllerDidAppear)
                                     category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryAdunit)
                                       param1: nil];
}
...
- (void)viewWillAppear: (BOOL)animated {
    ...
    if (_viewHandlers) {
        for (NSString *key in _viewHandlers) {
            ...
            if (viewHandler) {
                [viewHandler viewWillAppear: self
                                   animated: animated];
            }
        }
    }
...
- (void)viewWillDisappear: (BOOL)animated {
    ...
    if (_viewHandlers) {
        for (NSString *key in _viewHandlers) {
            ...
            if (viewHandler) {
                [viewHandler viewWillDisappear: self
                                      animated: animated];
            }
        }
    }

    [[USRVWebViewApp getCurrentApp] sendEvent: UADSNSStringFromAdUnitEvent(kUnityAdsViewControllerWillDisappear)
                                     category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryAdunit)
                                       param1: nil];
}
...
- (void)viewDidDisappear: (BOOL)animated {
    ...
    if (_viewHandlers) {
        for (NSString *key in _viewHandlers) {
            ...
            if (viewHandler) {
                [viewHandler viewDidDisappear: self
                                     animated: animated];
            }
        }
    }

    [[USRVWebViewApp getCurrentApp] sendEvent: UADSNSStringFromAdUnitEvent(kUnityAdsViewControllerDidDisappear)
                                     category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryAdunit)
                                       param1: nil];
}
...
- (void)setViews: (NSArray<NSString *> *)views {
    ...
    _currentViews = [[NSArray alloc] initWithArray: actualViews
                                         copyItems: true];
} /* setViews */
...
- (void)handleViewPlacement: (UIView *)view {
    if ([view superview] && [[view superview] isEqual: self.view]) {
        ...
        [view setCenter: [self.view convertPoint: self.view.center
...
- (UADSAdUnitViewHandler *)getViewHandler: (NSString *)viewName {
    ...
    if (_viewHandlers && [_viewHandlers objectForKey: viewName]) {
        ...
        if (viewHandler) {
            ...
            [_viewHandlers setObject: viewHandler
                              forKey: viewName];
        }
    }
