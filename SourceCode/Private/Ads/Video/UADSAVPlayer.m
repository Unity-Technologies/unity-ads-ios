        ...
        @try {
            [self.currentItem addObserver: self
                               forKeyPath: @"playbackLikelyToKeepUp"
                                  options: 0
                                  context: playbackLikelyToKeepUpKVOToken];
        } @catch (id exception) {
...
- (void)play {
    ...
    if (!self.isObservingCompletion) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver: self
                                                 selector: @selector(onCompletionListener:)
                                                     name: AVPlayerItemDidPlayToEndTimeNotification
...
- (void)seekTo: (long)msec {
    ...
    [self seekToTime: time
         ...
         [[USRVWebViewApp getCurrentApp] sendEvent: UADSNSStringFromAVPlayerEvent(kUnityAdsAVPlayerEventSeekTo)
                                          category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryVideoPlayer)
                                            param1: self.url, nil];
     }];
}
    ...
    if (context == itemStatusChangeToken) {
        ...
        if (playerItemStatus == AVPlayerItemStatusReadyToPlay) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [[USRVWebViewApp getCurrentApp] sendEvent: UADSNSStringFromAVPlayerEvent(kUnityAdsAVPlayerEventPrepared)
                                                 category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryVideoPlayer)
                                                   param1: self.url,
