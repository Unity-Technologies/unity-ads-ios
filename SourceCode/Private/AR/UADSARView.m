...
- (void)commonInit {
    ...
    [self initAR];
    self.mtkView = [[MTKView alloc] initWithFrame: self.frame
...
- (void)initAR {
    ...
    if (self.arSession) {
        [UADSARUtils session: self.arSession
                 setDelegate: self];
    }
}
...
- (void)addAnchor: (NSString *)identifier withTransform: (NSString *)transform {
    ...
    [UADSARUtils sessionAddAnchor: self.arSession
                           anchor: anchor];
    ...
    [jsAnchorIdsToObjCAnchorIds setValue: anchorId
                                  forKey: identifier];
    [objCAnchorIdsToJSAnchorIds setValue: identifier
                                  forKey: anchorId];
    [anchors setValue: anchor
               forKey: identifier];
} /* addAnchor */
...
- (void)removeAnchor: (NSString *)identifier {
    ...
    [UADSARUtils sessionRemoveAnchor: self.arSession
                              anchor: anchor];
...
- (void)session: (id)session didUpdateFrame: (id)frame {
    ...
    matrix_float4x4 viewMatrix = [UADSARUtils arCamera: camera
                              viewMatrixForOrientation               : interfaceOrientation];
...
- (NSString *)getPlanesString: (nonnull NSArray<id> *)anchors {
    ...
    for (int i = 0; i < anchors.count; i++) {
        ...
        [UADSARUtils arAnchorCenter: plane
                             center: anchorCenter];
        [UADSARUtils arAnchorExtent: plane
                             extent: anchorExtent];
