...
+ (void)WebViewExposed_sendPlacementContentReady: (NSString *)placementId withCallBack: (USRVWebViewCallback *)callback {
    ...
    if (delegate) {
        @try {
            ...
            [delegate placementContentReady: placementId
                           placementContent: placementContent];
            [callback invoke: nil];
        } @catch (NSException *e) {
            [callback error: NSStringFromPlacementContentDelegateError(kPlacementDelegateErrorDelegateDidError)
                       arg1: e];
        }
    } else {
        [callback error: NSStringFromPlacementContentDelegateError(kPlacementDelegateErrorDelegateNull)
                   arg1: nil];
    }
}
