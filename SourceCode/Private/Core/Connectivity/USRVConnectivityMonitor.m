...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info) {
    ...
    [USRVConnectivityMonitor sendToWebview: target
                                     flags: flags];
}
...
+ (void)sendToWebview: (SCNetworkReachabilityRef)reachabilityRef flags: (SCNetworkReachabilityFlags)flags {
    ...
    switch ([USRVConnectivityUtils networkStatusForFlags: flags]) {
        case NotReachable:
            [webViewApp sendEvent: Disconnected
                         category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryConnectivity)
                           param1: nil];
            break;

        case ReachableViaWiFi:
            [webViewApp sendEvent: Connected
                         category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryConnectivity)
                           param1: [NSNumber numberWithBool: TRUE], [NSNumber numberWithInt: 0], nil];
            break;

        case ReachableViaWWAN:
            [webViewApp sendEvent: Connected
                         category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryConnectivity)
                           param1: [NSNumber numberWithBool: FALSE], [NSNumber numberWithInteger: [USRVConnectivityUtils getNetworkType]], nil];
            break;
