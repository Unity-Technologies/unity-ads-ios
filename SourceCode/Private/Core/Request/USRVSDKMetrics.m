...
- (void)sendEventWithTags: (NSString *)event tags: (NSDictionary<NSString *, NSString *> *)tags {
    ...
    dispatch_async(self.metricQueue, ^{
        @try {
            ...
            if (tags != nil) {
                NSData *jsonTags = [NSJSONSerialization dataWithJSONObject: tags
                                                                   options: 0
                                                                     error: nil];
                tagString = [NSString stringWithFormat: @",\"t\":%@", [[NSString alloc] initWithData: jsonTags
            ...
            id<USRVWebRequest> request = [USRVWebRequestFactory create: self.metricEndpoint
                                                           requestType: @"POST"
                                                               headers: NULL
