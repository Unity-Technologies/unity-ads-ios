...
- (void)handleInvocation: (NSArray *)invocations {
    ...
    for (int idx = 0; idx < [invocations count]; idx++) {
        ...
        USRVWebViewCallback *webViewCallback = [[USRVWebViewCallback alloc] initWithCallbackId: callback
                                                                                  invocationId: [batch invocationId]];

        @try {
            [batch addInvocation: className
                      methodName: methodName
                      parameters: parameters
                        callback: webViewCallback];
        } @catch (NSException *exception) {
            USRVLogError(@"Error while adding invocation: %@", [exception reason]);
            [failedIndices setObject: exception
                              forKey: [NSNumber numberWithInt: idx]];
            [failedCallbacks setObject: webViewCallback
                                forKey: [NSNumber numberWithInt: idx]];
        }
    }
    ...
    for (int idx = 0; idx < [invocations count]; idx++) {
        if ([failedIndices objectForKey: [NSNumber numberWithInt: idx]]) {
            ...
            if (webViewCallback) {
                [webViewCallback error: [exception name]
                                  arg1: [exception reason], nil];
            }
        } else {
...
- (void)handleCallback: (NSDictionary *)callback {
    ...
    if ([parameters isKindOfClass: [NSString class]]) {
        ...
        parameters = [NSJSONSerialization JSONObjectWithData: [[callback objectForKey: @"parameters"] dataUsingEncoding: NSUTF8StringEncoding]
                                                     options: NSJSONReadingMutableContainers
                                                       error: &jsonError];
    }
    ...
    [USRVWebViewBridge handleCallback: callbackId
