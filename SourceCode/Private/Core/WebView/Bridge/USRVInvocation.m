...
+ (void)setClassTable: (NSArray<NSString *> *)allowedClasses {
    ...
    for (NSString *allowedClass in allowedClasses) {
        ...
        if (class) {
            [classTable setObject: class
                           forKey: allowedClass];

            for (int i = 0; i < count; i++) {
                SEL selector = method_getName(methods[i]);
                const char *mName = sel_getName(selector);
                NSString *currentSelectorName = [NSString stringWithCString: mName
                ...
                if (selector && [currentSelectorName rangeOfString: @"WebViewExposed_"].location == 0) {
                    ...
                    if (signature) {
                        ...
                        if (![selectorTable objectForKey: allowedClass]) {
                            [selectorTable setObject: [[NSMutableDictionary alloc] init]
                                              forKey: allowedClass];
                        }

                        if (![[selectorTable objectForKey: allowedClass] objectForKey: currentSelectorName]) {
                            [[selectorTable objectForKey: allowedClass] setObject: [[NSMutableDictionary alloc] init]
                                                                           forKey: currentSelectorName];
                        }
                        ...
                        [currentSelectorVersions setObject: selectorEntries
                                                    forKey: [NSNumber numberWithLong: numArgs]];
                        [[selectorTable objectForKey: allowedClass] setObject: currentSelectorVersions
                                                                       forKey: currentSelectorName];
                    }
                }
            }
...
- (void)addInvocation: (NSString *)className methodName: (NSString *)methodName parameters: (NSArray *)parameters callback: (USRVWebViewCallback *)callback {
    ...
    if (class && [selectorTable objectForKey: className]) {
        ...
        if (signature && selector) {
            ...
            [invocation setArgument: &callback
                            atIndex: 2 + [parameters count]];
