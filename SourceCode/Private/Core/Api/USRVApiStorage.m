...
+ (void)WebViewExposed_set: (NSString *)type key: (NSString *)key value: (id)value callback: (USRVWebViewCallback *)callback {
    ...
    if (storage && key && value) {
        ...
        if (success) {
            ...
            [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntSetValue)
                       arg1: key, value, nil];
        }
    } else {
        [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntGetStorage)
                   arg1: type, key, value, nil];
    }
} /* WebViewExposed_set */
...
+ (void)WebViewExposed_get: (NSString *)type key: (NSString *)key callback: (USRVWebViewCallback *)callback {
    ...
    if (storage && key) {
        ...
        if (value) {
            ...
            [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntGetValue)
                       arg1: key, nil];
        }
    } else {
        [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntGetStorage)
...
+ (void)WebViewExposed_write: (NSString *)type callback: (USRVWebViewCallback *)callback {
    ...
    if (storage) {
        ...
        if (success) {
            ...
            [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntWriteStorageToCache)
                       arg1: type, nil];
        }
    } else {
        [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntGetStorage)
...
+ (void)WebViewExposed_clear: (NSString *)type callback: (USRVWebViewCallback *)callback {
    ...
    if (storage) {
        ...
        if (success) {
            ...
            [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntClearStorage)
                       arg1: type, nil];
        }
    } else {
        [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntGetStorage)
...
+ (void)WebViewExposed_delete: (NSString *)type key: (NSString *)key callback: (USRVWebViewCallback *)callback {
    ...
    if (storage && key) {
        ...
        if (success) {
            ...
            [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntDeleteValue)
                       arg1: type, nil];
        }
    } else {
        [callback error: USRVNSStringFromStorageError(kUnityServicesCouldntGetStorage)
...
+ (UnityServicesStorageType)getStorageTypeFromString: (NSString *)typeString {
    ...
    if ([typeString isEqualToString: @"PUBLIC"]) typeValue = 1;
    else if ([typeString isEqualToString: @"PRIVATE"]) typeValue = 2;
