...
+ (void)WebViewExposed_download: (NSString *)url fileId: (NSString *)fileId headers: (NSArray *)headers append: (NSNumber *)append callback: (USRVWebViewCallback *)callback {
    if ([USRVConnectivityUtils getNetworkStatus] == NotReachable) {
        [callback error: USRVNSStringFromCacheError(kUnityServicesNoInternet)
                   arg1: nil];
        return;
    }
    ...
    if (!success) {
        [callback error: USRVNSStringFromCacheError(kUnityServicesFileAlreadyCaching)
                   arg1: nil];
    } else {
        [callback invoke: nil];
    }
...
+ (void)WebViewExposed_stop: (USRVWebViewCallback *)callback {
    if (![USRVCacheQueue hasOperations]) {
        [callback error: USRVNSStringFromCacheError(kUnityServicesNotCaching)
                   arg1: nil];
    } else {
        ...
        if (!error && contents) {
            if (encoding) {
                if ([encoding isEqualToString: @"UTF-8"]) {
                    fileContents = [[NSString alloc] initWithData: contents
                                                         encoding: NSUTF8StringEncoding];
                } else if ([encoding isEqualToString: @"Base64"]) {
                    fileContents = [contents base64EncodedStringWithOptions: 0];
                } else {
                    [callback error: USRVNSStringFromCacheError(kUnityServicesUnsupportedEncoding)
                               arg1: fileId, fileName, encoding, nil];
                    return;
                }
                ...
                [callback error: USRVNSStringFromCacheError(kUnityServicesUnsupportedEncoding)
                           arg1: fileId, fileName, [NSNull null], nil];
            }
        } else if (error) {
            [callback error: USRVNSStringFromCacheError(kUnityServicesFileIOError)
                       arg1: fileId, fileName, error.description, nil];
        } else if (!contents) {
            [callback error: USRVNSStringFromCacheError(kUnityServicesFileIOError)
                       arg1: fileId, fileName, "no file content", nil];
        }
    } else {
        [callback error: USRVNSStringFromCacheError(kUnityServicesFileNotFound)
                   arg1: fileId, fileName, nil];
    }
} /* WebViewExposed_getFileContent */
...
+ (void)WebViewExposed_setFileContent: (NSString *)fileId encoding: (NSString *)encoding content: (NSString *)content callback: (USRVWebViewCallback *)callback {
    ...
    if (encoding) {
        if ([encoding isEqualToString: @"UTF-8"]) {
            ...
            fileContents = [[NSData alloc] initWithBase64EncodedString: content
                                                               options: 0];
        } else {
            [callback error: USRVNSStringFromCacheError(kUnityServicesUnsupportedEncoding)
                       arg1: fileId, tagetFilePath, encoding, nil];
            return;
        }
    }
    ...
    @try {
        if (![[NSFileManager defaultManager] fileExistsAtPath: tagetFilePath]) {
            [[NSFileManager defaultManager] createFileAtPath: tagetFilePath
                                                    contents: nil
                                                  attributes: nil];
        }
        ...
        [fileContents writeToFile: tagetFilePath
                       atomically: YES];
    } @catch (NSException *exception) {
        [callback error: USRVNSStringFromCacheError(kUnityServicesFileIOError)
                   arg1: fileId, tagetFilePath, exception.reason, nil];
        return;
    }
    [callback invoke: nil];
} /* WebViewExposed_setFileContent */
...
+ (void)WebViewExposed_getFiles: (USRVWebViewCallback *)callback {
    ...
    NSError *error;
    NSArray *dirContents = [fm contentsOfDirectoryAtPath: [USRVSdkProperties getCacheDirectory]
                                                   error: &error];
...
+ (void)WebViewExposed_getVideoInfo: (NSString *)fileId callback: (USRVWebViewCallback *)callback {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath: fileName]) {
        @try {
            ...
            [callback error: USRVNSStringFromCacheError(kUnityServicesInvalidArgument)
                       arg1: nil];
        }
    } else {
        [callback error: USRVNSStringFromCacheError(kUnityServicesFileIOError)
...
+ (void)WebViewExposed_deleteFile: (NSString *)fileId callback: (USRVWebViewCallback *)callback {
    NSString *fileName = [NSString stringWithFormat: @"%@", [USRVApiCache fileIdToFilename: fileId]];
    NSError *error;
    BOOL removed = [[NSFileManager defaultManager] removeItemAtPath: fileName
                                                              error: &error];
...
+ (void)WebViewExposed_getHash: (NSString *)stringToHash callback: (USRVWebViewCallback *)callback {
    [callback invoke: [stringToHash uads_sha256], nil];
}
...
+ (NSDictionary *)getFileDictionary: (NSString *)fileId {
    NSString *fileName = [USRVApiCache fileIdToFilename: fileId];
    NSError *error;
    NSDictionary *fileAttribs = [[NSFileManager defaultManager] attributesOfItemAtPath: fileName
                                                                                 error: &error];
    ...
    if (!error) {
        ...
        [resultDict setObject: fileId
                       forKey: @"id"];
        [resultDict setObject: [NSNumber numberWithLongLong: [modifiedDate timeIntervalSince1970] * 1000]
                       forKey: @"mtime"];
        [resultDict setObject: [NSNumber numberWithBool: true]
                       forKey: @"found"];
        [resultDict setObject: size
                       forKey: @"size"];
    } else {
        [resultDict setObject: [NSNumber numberWithBool: false]
                       forKey: @"found"];
    }
