...
+ (void)WebViewExposed_prepare: (NSDictionary *)parameters prepareTimeout: (NSNumber *)timeout callback: (USRVWebViewCallback *)callback {
    [[USTRStore appSheet] prepareAppSheet: parameters
                  prepareTimeoutInSeconds: timeout.intValue / 1000
                          completionBlock: ^(BOOL result, NSString *_Nullable error) {
                              id webViewApp = [USRVWebViewApp getCurrentApp];

                              if (result) {
                                  if (webViewApp) {
                                      [webViewApp sendEvent: USRVNSStringFromAppSheetEvent(kAppSheetPrepared)
                                                   category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryAppSheet)
                                                     param1: parameters, nil];
                                  }
                              } else {
                                  if (webViewApp) {
                                      [webViewApp sendEvent: USRVNSStringFromAppSheetEvent(kAppSheetFailed)
                                                   category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryAppSheet)
                                                     param1: error, parameters, nil];
                                  }
                              }
                          }];
    [callback invoke: nil];
} /* WebViewExposed_prepare */
...
+ (void)WebViewExposed_prepareImmediate: (NSDictionary *)parameters prepareTimeout: (NSNumber *)timeout callback: (USRVWebViewCallback *)callback {
    [[USTRStore appSheet] prepareAppSheetImmediate: parameters
                           prepareTimeoutInSeconds: timeout.intValue / 1000
                                   completionBlock: ^(BOOL result, NSString *_Nullable error) {
                                       id webViewApp = [USRVWebViewApp getCurrentApp];
...
+ (void)WebViewExposed_presentWithTopViewControllerSupport: (NSDictionary *)parameters animated: (NSNumber *)animated callback: (USRVWebViewCallback *)callback {
    [[USTRStore appSheet] presentAppSheetWithTopViewControllerSupport: parameters
                                                             animated: [animated boolValue]
                                                      completionBlock: ^(BOOL result, NSString *_Nullable error) {
                                                          if (result) {
                                                              [callback invoke: parameters, nil];
...
+ (void)WebViewExposed_destroy: (NSDictionary *)parameters callback: (USRVWebViewCallback *)callback {
    if ([[USTRStore appSheet] destroyAppSheet: parameters]) {
        ...
        [callback error: USRVNSStringFromAppSheetError(kUnityServicesAppSheetErrorNoAppSheetFound)
                   arg1: parameters, nil];
    }
}
