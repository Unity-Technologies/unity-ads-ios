...
- (void)prepareAppSheet: (NSDictionary *)parameters prepareTimeoutInSeconds: (int)timeout completionBlock: (nullable void (^)(BOOL result, NSString *__nullable error))block {
    ...
    if ([self getCachedController: iTunesId]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [viewController loadProductWithParameters: appSheetParameters
                                      completionBlock: ^(BOOL result, NSError *error) {
                                          ...
                                          if (result) {
                                              ...
                                              [self.appSheetCache setValue: viewController
                                                                    forKey: iTunesId];
                                              block(true, nil);
                                          } else {
...
- (void)presentAppSheet: (NSDictionary *)parameters animated: (BOOL)animated completionBlock: (nullable void (^)(BOOL result, NSString *__nullable error))block {
    ...
    if (cachedController) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [[UADSApiAdUnit getAdUnit] presentViewController: cachedController
                                                    animated: animated
                                                  completion: ^{
                                                      if ([USRVWebViewApp getCurrentApp]) {
                                                          [[USRVWebViewApp getCurrentApp] sendEvent: USRVNSStringFromAppSheetEvent(kAppSheetOpened)
                                                                                           category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryAppSheet)
                                                                                             param1: parameters, nil];
                                                      }
                                                  }];
        });
...
- (void)presentAppSheetWithTopViewControllerSupport: (NSDictionary *)parameters animated: (BOOL)animated completionBlock: (nullable void (^)(BOOL result, NSString *__nullable error))block {
    ...
    self.presentingAnimated = animated;
    dispatch_async(dispatch_get_main_queue(), ^{
        [presentingController presentViewController: cachedController
                                           animated: animated
                                         completion: ^{
                                             if ([USRVWebViewApp getCurrentApp]) {
                                                 [[USRVWebViewApp getCurrentApp] sendEvent: USRVNSStringFromAppSheetEvent(kAppSheetOpened)
    ...
    block(true, nil);
} /* presentAppSheetWithTopViewControllerSupport */
...
- (NSDictionary *)getSanitizedParametersFromParameters: (NSDictionary *)parameters {
    ...
    if ([[mutableParameters objectForKey: @"adNetworkNonceString"] isKindOfClass: [NSString class]]) {
        [mutableParameters setObject: [[NSUUID alloc] initWithUUIDString: [mutableParameters objectForKey: @"adNetworkNonceString"]]
                              forKey: @"adNetworkNonce"];
...
- (void)productViewControllerDidFinish: (SKStoreProductViewController *)viewController {
    ...
    [viewController.presentingViewController dismissViewControllerAnimated: self.presentingAnimated
                                                                completion: ^{}];
}
...
- (void)sendAppSheetClosed {
    ...
    [[USRVWebViewApp getCurrentApp] sendEvent: USRVNSStringFromAppSheetEvent(kAppSheetClosed)
                                     category: USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryAppSheet)
                                       param1: self.presentingParameters, nil];
}
