...
- (instancetype)initWithPlacementId:(NSString *)placementId size:(CGSize)size {
    ...
    if (self) {
        ...
        _bannerView = [[UADSBannerView alloc] initWithPlacementId:placementId size:size];
...
- (void)constructView {
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[bannerView]|" options:0 metrics:nil views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[bannerView]|" options:0 metrics:nil views:views]];
}
...
- (void)createNewTimer:(NSDate *)fireDate {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:self.refreshRate target:bannerRefreshBlock selector:@selector(block) userInfo:nil repeats:YES];
...
- (void)bannerViewDidError:(UADSBannerView *)bannerView error:(UADSBannerError *)error {
    if (self.delegate && [self.delegate respondsToSelector:@selector(unityAdsRefreshBannerDidError:message:)]) {
        __weak UADSBannerAdRefreshView *weakSelf = self;
        [self.delegate unityAdsRefreshBannerDidError:weakSelf message:error.localizedDescription];
    }
}
