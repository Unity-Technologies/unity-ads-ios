...
- (void)setupConstraints {
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[banner]|" options:0 metrics:nil views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[banner]|" options:0 metrics:nil views:views]];
}
...
- (void)didMoveToSuperview {
    if (_constraints == nil && self.superview) {
        _constraints = [self getBannerConstraints:self.bannerPosition banner:self.bannerAdRefreshView superView:self.superview];
...
- (NSArray<NSLayoutConstraint *> *)getBannerConstraints:(UnityAdsBannerPosition)bannerPosition banner:(UIView *)banner superView:(UIView *)superView {
    ...
    switch (bannerPosition) {
        ...
        case kUnityAdsBannerPositionTopLeft:
            [constraints addObject:[NSLayoutConstraint constraintWithItem:banner attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:superView attribute:NSLayoutAttributeLeft multiplier:1.0 constant:0]];
            break;
        case kUnityAdsBannerPositionBottomRight:
        case kUnityAdsBannerPositionTopRight:
            [constraints addObject:[NSLayoutConstraint constraintWithItem:banner attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:superView attribute:NSLayoutAttributeRight multiplier:1.0 constant:0]];
            break;
