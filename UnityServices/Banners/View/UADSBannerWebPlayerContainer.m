...
- (instancetype)initWithBannerAdId:(NSString *)bannerAdId webPlayerSettings:(NSDictionary *)webPlayerSettings webPlayerEventSettings:(NSDictionary *)webPlayerEventSettings size:(CGSize)size {
    ...
    if (self) {
        ...
        self.webPlayer = [[UADSWebPlayerView alloc] initWithFrame:CGRectZero viewId:bannerAdId webPlayerSettings:webPlayerSettings];
        ...
        [[UADSWebPlayerViewManager sharedInstance] addWebPlayerView:self.webPlayer viewId:bannerAdId];
        [self constructView];
    }
    return self;
}
...
- (void)constructView {
    ...
    self.heightConstraint = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:self.size.height];
    self.widthConstraint = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:self.size.width];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[webPlayer]|" options:0 metrics:nil views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[webPlayer]|" options:0 metrics:nil views:views]];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (object == self) {
        if ([keyPath isEqualToString:@"alpha"] || [keyPath isEqualToString:@"frame"]) {
            ...
            [USRVBannerBridge bannerDidResizeWithBannerId:self.bannerAdId frame:frame alpha:self.alpha];
        } else if ([keyPath isEqualToString:@"hidden"]) {
            if (self.hidden) {
                [USRVBannerBridge bannerVisibilityChangedWithBannerId:self.bannerAdId bannerVisibility:UADSBannerVisibilityGone];
            } else {
                [USRVBannerBridge bannerVisibilityChangedWithBannerId:self.bannerAdId bannerVisibility:UADSBannerVisibilityVisible];
            }
        }
    }
}
