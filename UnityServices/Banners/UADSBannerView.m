...
- (instancetype)initWithPlacementId:(NSString *)placementId size:(CGSize)size {
    ...
    if (self) {
        ...
        [[UADSBannerViewManager sharedInstance] addBannerView:self bannerAdId:self.viewId];
...
- (void)bridgeLoad {
    [USRVBannerBridge loadBannerPlacement:self.placementId bannerAdId:self.viewId size:self.size];
}
...
- (void)setBannerWebPlayerContainer:(UADSBannerWebPlayerContainer *)bannerWebPlayerContainer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf) {
            ...
            [weakSelf addConstraint:[NSLayoutConstraint constraintWithItem:bannerWebPlayerContainer attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:weakSelf attribute:NSLayoutAttributeCenterX multiplier:1.0 constant:0]];
            [weakSelf addConstraint:[NSLayoutConstraint constraintWithItem:bannerWebPlayerContainer attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:weakSelf attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:0]];
        }
    });
}
...
- (void)setupConstraints {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [weakSelf addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:[bannerView(width)]" options:0 metrics:metrics views:views]];
        [weakSelf addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[bannerView(height)]" options:0 metrics:metrics views:views]];
...
- (void)sdkInitializeFailed:(NSError *)error {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(bannerViewDidError:error:)]) {
        ...
        __weak UADSBannerView *weakSelf = self;
        [self.delegate bannerViewDidError:weakSelf error:error];
    }
}
