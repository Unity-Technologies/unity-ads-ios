...
+ (void)sendFrameUpdate:(NSString *)viewId frame:(CGRect)frame alpha:(CGFloat)alpha {
    ...
    if (currentApp) {
        ...
        [currentApp sendEvent:UADSNSStringFromWebPlayerEvent(kUnityAdsWebPlayerFrameUpdate) category:USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryWebPlayer) params:@[viewId, x, y, width, height, alphaNumber]];
    }
}
...
+ (void)sendGetFrameResponse:(NSString *)callId viewId:(NSString *)viewId frame:(CGRect)frame alpha:(CGFloat)alpha {
    ...
    if (currentApp) {
        ...
        NSNumber *height = [NSNumber numberWithFloat:frame.size.height];
        NSNumber *alphaNumber = [NSNumber numberWithFloat:alpha];
        [currentApp sendEvent:UADSNSStringFromWebPlayerEvent(kUnityAdsWebPlayerGetFrameResponse) category:USRVNSStringFromWebViewEventCategory(kUnityServicesWebViewEventCategoryWebPlayer) params:@[callId, viewId, x, y, width, height, alphaNumber]];
    }
}
