...
initializationDelegate:(nullable id<UnityAdsInitializationDelegate>)initializationDelegate {
    ...
    if ([USRVSdkProperties getCurrentInitializationState] != NOT_INITIALIZED) {
        ...
        if (![previousGameId isEqualToString:gameId]) {
            differingParams = [NSString stringWithFormat:@"%@%@", differingParams, [self createExpectedParametersString:@"Game ID" current:previousGameId received:gameId]];
        }
        
        bool previousTestMode = [USRVSdkProperties isTestMode];
        if (previousTestMode != testMode) {
            differingParams = [NSString stringWithFormat:@"%@%@", differingParams, [self createExpectedParametersString:@"Test Mode" current:[NSString stringWithFormat:@"%d", previousTestMode] received:[NSString stringWithFormat:@"%d", testMode]]];
        }
        
        bool previousLoadEnabled = [USRVSdkProperties isPerPlacementLoadEnabled];
        if (previousLoadEnabled != usePerPlacementLoad) {
            differingParams = [NSString stringWithFormat:@"%@%@", differingParams, [self createExpectedParametersString:@"Enable Per Placement Load" current:[NSString stringWithFormat:@"%d", previousLoadEnabled] received:[NSString stringWithFormat:@"%d", usePerPlacementLoad]]];
        }
        ...
        if ([differingParams length] > 0) {
            ...
            if (delegate != nil && [delegate respondsToSelector:@selector(unityServicesDidError:withMessage:)]) {
                [delegate unityServicesDidError:kUnityServicesErrorInvalidArgument withMessage:errorMessage];
            }
            if (initializationDelegate != nil && [delegate respondsToSelector:@selector(initializationFailed:withMessage:)]) {
                [initializationDelegate initializationFailed:kUnityInitializationErrorInvalidArgument withMessage:errorMessage];
            }
            return;
        }
    }
    ...
    if ([USRVSdkProperties getCurrentInitializationState] == INITIALIZED_FAILED) {
        [USRVSdkProperties notifyInitializationFailed:kUnityInitializationErrorInternalError withErrorMessage:@"Unity Ads SDK failed to initialize due to previous failed reason."];
        return;
    }
    ...
    if (!gameId || [gameId length] == 0) {
        ...
        [USRVSdkProperties notifyInitializationFailed:kUnityInitializationErrorInvalidArgument withErrorMessage:errorMessage];
        if (delegate != nil && [delegate respondsToSelector:@selector(unityServicesDidError:withMessage:)]) {
            [delegate unityServicesDidError:kUnityServicesErrorInvalidArgument withMessage:errorMessage];
        }
        return;
    ...
    @synchronized (self) {
        ...
        if ([USRVEnvironmentProperties isEnvironmentOk]) {
            ...
            [USRVSdkProperties notifyInitializationFailed:kUnityInitializationErrorInternalError withErrorMessage:@"Unity Ads SDK failed to initialize due to environment check failed."];
            return;
        }
