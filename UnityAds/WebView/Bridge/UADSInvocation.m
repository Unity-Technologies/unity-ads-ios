...
- (void)addInvocation:(NSString *)className methodName:(NSString *)methodName parameters:(NSArray *)parameters callback:(UADSWebViewCallback *)callback {
    ...
    if (class && [selectorTable objectForKey:className]) {
        ...
        if (!class) {
            exceptionReason = [NSString stringWithFormat:@"Could not fetch class from allowed classes cache"];
        }
...
- (BOOL)nextInvocation {
    if (self.invocations && [self.invocations count] > 0) {
        ...
        [UADSWebViewBridge handleInvocation:invocation];
...
- (void)sendInvocationCallback {
    ...
    [[UADSWebViewApp getCurrentApp] invokeCallback:self];
}
